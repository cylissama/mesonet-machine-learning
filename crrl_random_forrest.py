...
class CRRLFeatureAnalysis:
    """
    Simplified Random Forest feature significance analysis for CRRL site only
    """

    def __init__(self, data, target_variable, output_dir=None):
        """
        Initialize with CRRL mesonet dataset
        ...
        if output_dir is None:
            ...
            self.output_dir = f"CRRL_{target_variable}_analysis_{timestamp}"
        else:
    ...
    def prepare_data(self, remove_outliers=True, outlier_threshold=3):
        """
        Prepare CRRL data for analysis
        """
        print("\nPreparing CRRL data...")
    ...
    def run_random_forest_analysis(self, n_estimators=100, test_size=0.2, random_state=42):
        ...
        """
        print(f"\n{'=' * 60}")
        print(f"CRRL RANDOM FOREST ANALYSIS - {self.target_variable}")
        print(f"{'=' * 60}")
    ...
    def create_visualizations(self, top_n=20):
        ...
        fig.suptitle(f'CRRL Feature Importance Analysis - {self.target_variable}', fontsize=16)
        ...
        # Save figure
        fig_path = os.path.join(self.output_dir, 'figures', f'CRRL_{self.target_variable}_analysis.png')
    ...
    def save_results(self):
        ...
        importance_file = os.path.join(self.output_dir, 'data', f'CRRL_{self.target_variable}_feature_importance.csv')
        ...
        summary_file = os.path.join(self.output_dir, f'CRRL_{self.target_variable}_summary.json')
        with open(summary_file, 'w') as f:
            json.dump(summary, f, indent=2)
        print(f"Saved summary: {summary_file}")

        # Save trained model
        model_file = os.path.join(self.output_dir, f'CRRL_{self.target_variable}_model.pkl')
    ...
    def _create_text_summary(self):
        ...
        text_lines.append(f"CRRL FEATURE SIGNIFICANCE ANALYSIS")
        ...
        # Save text summary
        text_file = os.path.join(self.output_dir, f'CRRL_{self.target_variable}_summary.txt')
        with open(text_file, 'w') as f:
            f.write('\n'.join(text_lines))
        print(f"Saved text summary: {text_file}")


def analyze_crrl_site(data_file, target_variable, output_dir=None):
    """
    Main function to analyze CRRL site
    ...
    # Initialize analyzer
    analyzer = CRRLFeatureAnalysis(data, target_variable, output_dir)
...
if __name__ == "__main__":
    # Analyze TAIR-VT20 for CRRL
    print("Analyzing TAIR-VT20 for CRRL site...")
    analyzer1, importance1, summary1 = analyze_crrl_site(
        '/Users/cylis/Work/mes_summer25/RFSM mods/RFSM_mesonet_data_filled.csv',  # Replace with your file path
        'TAIR_VT20'
    )

    # Analyze VT90-VT20 for CRRL
    print("\nAnalyzing VT90-VT20 for CRRL site...")
    analyzer2, importance2, summary2 = analyze_crrl_site(
        '/Users/cylis/Work/mes_summer25/RFSM mods/RFSM_mesonet_data_filled.csv',  # Replace with your file path
        'VT90_VT20'
    )
